...
def occlusions(flow0, frame0, frame1):
	...
	y1 = flow0[ty][tx][1]
	print(flow0[ty][tx])
	print(flow1[int(ty + y1)][int(tx+x1)])
	...
	for y in range (height):
		for x in range (width):
			...
			if(int(x+x1) > width-1) or (int(y+y1) > height-1):
				if(occ1t[y][x] == 0):
					if f1 == 0:
						print("ERROR assigning 1 to what should be a 0 part1!")
						f1 = 1
					count+=1
				occ1[y][x] = 1
				
			elif(np.sum(np.abs(flow0[y][x] - flow1[int(y + y1)][int(x+x1)])) > 0.5):
				if(occ1t[y][x] == 0):
					if f2 == 0:
						print("ERROR assigning 1 to what should be a 0!")
						f2 = 1
					count+=1
				occ1[y][x] = 1
				
				
			if(np.sum(np.abs(flow1[y][x])) > height+width):
				occ0[y][x] = 1
	print(count)
	occ0 = pickle.load(open('occ0.step5.sample', 'rb')) # load sample result
	occ1 = pickle.load(open('occ1.step5.sample', 'rb')) # load sample result
	return occ0,occ1
...
def interpflow(flow, frame0, frame1, t):
	...
	'''
	iflow=np.full((len(flow),len(flow[0]),2),10.0**10)
	h,w,_ = flow.shape

	f = pickle.load(open('flow1.step4.sample', 'rb'))
				...
				if(x%int(x) >= .5):
					if(y%int(y) >= .5):
						try:
							xc = int(round(x))
							yc = int(round(y))
							if np.sum(iflow[yc][xc]) >= 10**10:
								iflow[yc][xc] = flow[i][j]
							else:
								a = np.sum(np.abs(frame0[i][j] - frame1[int(round(flow[i][j][1]))][int(round(flow[i][j][0]))]))  
								b = np.sum(np.abs(frame0[i][j] - frame1[int(round(iflow[yc][xc][1]))][int(round(iflow[yc][xc][0]))]))
								if a < b:
									iflow[yc][xc] = flow[i][j]
						except:
							count = 0
